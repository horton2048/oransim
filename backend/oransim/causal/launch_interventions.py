...
def run_intervention(
    ...
    if name == "price_up_30":
        ...
        return InterventionResult(
            name,
            res.total_kpis,
            _kpi_delta(base_kpi, res.total_kpis),
            label="价格 +30%",
            note="价格弹性 (图级 do(price_point))",
        )
    ...
    if name == "organic_only":
        ...
        return InterventionResult(
            name,
            dict(fc_org.per_type_totals),
            delta,
            paid_events=paid_events,
            label="纯自然量",
            note="treatment_boost_factor=0；paid 事件为 0",
            extra={"organic_seed_len": len(organic_seed), "full_total": fc_full.per_type_totals},
        )
