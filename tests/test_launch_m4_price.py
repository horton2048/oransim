...
def test_at_m4_01_aov_param_snapshot_unchanged():
    ...
    assert math.isclose(kpi_new["revenue"], revenue_expected, rel_tol=1e-7), (
...
def test_at_m4_03_price_feature_natural_degradation():
    ...
    oc_2x = ag.simulate(imp, creative, rng_seed=5, price_cny=ref_price * 2, reference_price=ref_price)

    # None 与 reference 等价：逐 agent convert_prob 相等
    np.testing.assert_array_almost_equal(
        oc_none.convert_prob, oc_ref.convert_prob, decimal=8,
        err_msg="price_cny=None 与 price_cny=reference_price 的 convert_prob 应相等"
    )

    # 2× reference 产生不同（更低）convert_prob
    assert not np.allclose(oc_none.convert_prob, oc_2x.convert_prob), (
        "price_cny=2×ref 的 convert_prob 应与 None/ref 不同（价格负效应）"
    )
    # 方向验证：高价 → 均值 convert_prob 降低（高价格 → 负特征 → 负 logit → 低概率）
    assert np.mean(oc_2x.convert_prob) < np.mean(oc_none.convert_prob), (
...
def test_at_m4_04_price_monotonicity():
    ...
    base_rev = sess.current_result.total_kpis["revenue"]
    ...
    assert 0 < conv_dn - base_conv, "上升幅度 > 0"
...
def test_at_m4_05_price_budget_commutativity():
    ...
    assert math.isclose(rev_a, rev_b, rel_tol=1e-6), (
...
def test_at_m4_06_price_elif_uses_current_conversions():
    ...
    conv_after_budget = sess.current_result.total_kpis["conversions"]
    ...
    assert math.isclose(rev_after_price, expected_revenue, rel_tol=1e-6), (
    ...
    assert not math.isclose(rev_after_price, old_rev, rel_tol=1e-4), (
...
def test_at_m4_08_decision_weights_frozen():
    ...
    np.testing.assert_array_equal(StatisticalAgents.W_CLICK, expected_w_click,
                                  err_msg="W_CLICK 被修改，违反权重冻结（铁律 4）")
    np.testing.assert_array_equal(StatisticalAgents.W_ENGAGE, expected_w_engage,
                                  err_msg="W_ENGAGE 被修改，违反权重冻结（铁律 4）")
    np.testing.assert_array_equal(StatisticalAgents.W_CONVERT, expected_w_convert,
                                  err_msg="W_CONVERT 被修改，违反权重冻结（铁律 4）")

    # 新增价格系数必须存在且为负（高价 → 负效应）
    assert hasattr(StatisticalAgents, "W_PRICE_SENS"), (
