# replay-viz — oransim 前端样板（操作台 + 战况回放）

oransim 推演结果的产品前端定稿样板。**纯前端、不连后端、内嵌真实响应样本**：操作台输入投放 brief → 预演 → 结果仪表盘 → 一键进入电影式战况回放。形式已定稿（2026-06-12），历史探索材料（v1/v2 原型、maya 前作、调研归档）已清理，备份在 `%TEMP%\replay-viz-pre-cleanup-2026-06-12.zip`。
